...
class Mlp(nn.Module):
    ...
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.,
                 mlp_search=False, finetune=False, mlp_threshold=None,
                 token_search=False, token_threshold=None):
        super().__init__()
        ...
        # 此处是对mlp_ratio进行搜索，但结果不好，所以进行尝试mlp隐藏层神经元个数的搜索

        # original_ratio = int(hidden_features / in_features)
        # self.width = int(in_features / 2)
        # num = original_ratio * 2

        # print(self.width)
        # print(original_ratio)
        self.mlp_search = mlp_search
        # if self.mlp_search:
        #     self.mlp_zeta = nn.Parameter(torch.ones(1, 1, num))

        if self.mlp_search:
            self.mlp_zeta = nn.Parameter(torch.ones(1, 1, hidden_features))
        self.finetune = finetune
        self.threshold = mlp_threshold
    ...
    def forward(self, x):
        ...
        x = self.fc1(x)
        # adavit采用就是在此处进行token的mask，所以此处也采用这种办法
        # 此处传进来的token_zeta的形状为(1,num_patch,1)
        # 此处的x的形状为(B,N,D)

        x = self.act(x)
        # 但由于0值激活之后就不是0了所以先在这里进行token的选择
        # 此处传进来的token_zeta的形状为(1,num_patch,1)
        # 此处的x的形状为(B,N,D)
        # if self.token_search:
        #     if token_zeta is None:
        #         print("mlp中的token——zeta为空！！！！！！！！！")
        #     x_cls = x[:, 0:1, :]
        #     # print("mlp中的cls_x的形状为：", x_cls.shape)
        #     x_other_token = x[:, 1:, :]
        #     if not self.finetune:
        #         x_other_token *= token_zeta
        #         print("进行mlp中的token搜索")
        #     else:
        #         print("进行mlp中token的第一次mask")
        #         token_cfg = (token_zeta > self.token_threshold).int()
        #         x_other_token *= token_cfg
        #
        #     x = torch.cat([x_cls, x_other_token], dim=1)
        #     # print("拼接结束后的x的形状为：", x.shape)

        if self.mlp_search:
            # print(self.mlp_zeta.unsqueeze(-1).shape)
            # mlp_zeta = self.mlp_zeta.unsqueeze(-1).expand(-1, -1, -1, self.width).reshape(1, 1, -1)
            # print(mlp_zeta.shape)
            # print(mlp_zeta)
            if not self.finetune:
                x *= self.mlp_zeta
                # print('进行mlp_zeta的学习')
            else:
                mlp_cfg = (self.mlp_zeta > self.threshold).int()
                # print(mlp_cfg,mlp_cfg.shape)
                x *= mlp_cfg
                # print('进行mlp的mask')
        ...
        x = self.drop2(x)
        # print(x.shape)
        # if self.token_search and self.finetune:
        #     print("进行第二次mlp的token的mask")
        #     cls_cfg = token_cfg.new_ones((1, 1, 1))
        #     full_token_cfg = torch.cat([cls_cfg, token_cfg], dim=1)
        #     print("mlp的第二次cfg的形状为：", full_token_cfg.shape)
        #     x *= full_token_cfg
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.,
                 finetune=False, head_threshold=None, head_search=True):
        # token_search=False, token_threshold=None):
        super().__init__()
        ...
        if self.head_search:
            self.head_zeta = nn.Parameter(torch.ones(1, 1, self.num_heads, 1, 1))
        self.finetune = finetune
        self.threshold = head_threshold
        #
        # self.token_search = token_search
        # self.token_threshold = token_threshold
        # # self.token_zeta = token_zeta
    ...
    def forward(self, x):
        ...
        # if self.token_search:
        #     if token_zeta is None:
        #         print("MHSA中的token_zeta为空！！！！")
        #     #     对token先进行处理，传进来的token_zeta的形状为(1,num_patch,1),
        #     #                                 而qkv的形状为(3,B,H,N,H_dim)
        #     # 首先需要解决对qkv的cls_token进行剥离
        #     qkv_cls = qkv[:, :, :, 0:1, :]
        #     # print("qkv_cls的形状为：", qkv_cls.shape)
        #     qkv_other_token = qkv[:, :, :, 1:, :]
        #     # print("qkv_other_token的形状为：", qkv_other_token.shape)
        #
        #     # 然后解决qkv乘上token的重要性,最好不直接重新赋值
        #     if not self.finetune:
        #         qkv_other_token *= token_zeta.unsqueeze(0).unsqueeze(0)
        #         print("进行MHSA中token的学习")
        #     else:
        #         token_cfg = (token_zeta.unsqueeze(0).unsqueeze(0) > self.token_thresold).int()
        #         print("对qkv的token进行稀疏化")
        #         qkv_other_token *= token_cfg
        #
        #     qkv = torch.cat([qkv_cls, qkv_other_token], dim=3)
        #     print("qkv处理结束后的形状为：", qkv.shape)
        ...
        if self.head_search:
            if not self.finetune:
                ...
                # self.head_zeta.requires_grad = False
                head_cfg = (self.head_zeta > self.threshold).int()
                # head_cfg = (self.head_zeta < self.threshold).int()
                # 验证实验此处改成<,正常实验此处改为>
                # print(head_cfg,type(head_cfg))
                # print(self.head_zeta)
        ...
        # 此时对于attn矩阵是对应的行和列为0，以及部分头的整个(N,N)的块为0，为了防止行为0在softmax对其他的块进行干扰，
        # 需要在softmax之前对这些行进行特殊处理
        # 而块就不需要，因为后面乘以v就统一变成0了
        # 注意此时的attn矩阵的形状为(B,H,N,N)
        # if self.token_search and self.finetune:
        #     token_mask = (token_zeta > self.token_threshold)
        #     cls_token_mask = token_mask.new_ones(1, 1, 1).bool()
        #     mask = torch.cat([cls_token_mask, token_mask], dim=1)
        #     # 现在的mask的形状为(1,N,1)
        #     print("对attn的第二次mask的形状为:", mask.shape)
        #     mask = mask.view(1, 1, N, 1) * mask.view(1, 1, 1, N)
        #     attn[~mask] = -1e6
        ...
        x = self.proj(x)
        x = self.proj_drop(x)
        # 在此也需要消除mask——token带来的影响
        # if self.token_search and self.finetune:
        #     token_mask = (token_zeta > self.token_threshold).int()
        #     cls_token_mask = token_mask.new_ones(1, 1, 1)
        #     mask = torch.cat([cls_token_mask, token_mask], dim=1)
        #     x *= mask
        return x
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
                 ...
                 finetune=False, head_threshold=None, head_search=False,
                 mlp_threshold=None, mlp_search=False):
        # token_threshold=None, token_search=False, num_patches=None):
        super().__init__()
        self.norm1 = norm_layer(dim)

        # self.token_search = token_search
        # self.token_zeta = None
        # if self.token_search:
        #     self.token_zeta = nn.Parameter(torch.ones(1, num_patches, 1))
        # else:
        #     self.token_zeta = None

        self.attn = Attention(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop,
                              finetune=finetune, head_threshold=head_threshold, head_search=head_search)
        # token_search=token_search, token_threshold=token_threshold)
        ...
        self.mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop,
                       finetune=finetune, mlp_threshold=mlp_threshold, mlp_search=mlp_search, )
        # token_search=token_search, token_threshold=token_threshold)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 finetune=False,
                 head_threshold=None, mlp_threshold=None,
                 # token_threshold=None,
                 mlp_search=False, head_search=False):
        # , token_search=False):
        """
        Args:
        ...
        self.blocks = nn.Sequential(*[
            Block(
                ...
                finetune=finetune, head_threshold=head_threshold, mlp_threshold=mlp_threshold, mlp_search=mlp_search,
