...
def get_head_zeta(model):
    ...
    for name, param in model.named_parameters():
        if 'head_zeta' in name:
            head_zeta.append(param.cpu().detach().reshape(-1).numpy().tolist())  # reshape(-1）是将一个多维矩阵改成一长行

    head_zetas = [z for k in head_zeta for z in k]  # 进一步展平，展平为一行
    return head_zetas
...
def get_mlp_zeta(model):
    ...
    for name, param in model.named_parameters():
        if 'mlp_zeta' in name:
            mlp_zeta.append(param.cpu().detach().reshape(-1).numpy().tolist())  # reshape(-1）是将一个多维矩阵改成一长行

    mlp_zetas = [z for k in mlp_zeta for z in k]  # 进一步展平，展平为一行
    return mlp_zetas
...
def get_threshold(checkpoint_path, head_prune_ratio, mlp_prune_ratio):
    ...
    print('The search epoch of the completed file is:', checkpoint['epoch'])
    head_zeta = []
    mlp_zeta = []
    head_threshold = None
    mlp_threshold = None
